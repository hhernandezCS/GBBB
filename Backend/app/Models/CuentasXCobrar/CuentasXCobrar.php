...
class CuentasXCobrar extends Model {
    ...
    {
        $recibos = $this->select(['id_cuentaxcobrar', 'id_tipo_documento', 'no_documento_origen', 'fecha_movimiento', 'descripcion_movimiento', 'id_cliente',
            'monto_movimiento', 'monto_movimiento_me', 'saldo_actual',
            'saldo_actual_me', 'fecha_vencimiento', 'estado']);
    ...
    public function obtenerCuentasCliente($request)
    {
        ...
        if ((!empty($request->id_cliente))) {
            ...
            if(!empty($request->id_tipo_documento)){
                $productos->where('id_tipo_documento' , $request->id_tipo_documento);
            }
