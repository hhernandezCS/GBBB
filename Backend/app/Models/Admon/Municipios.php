...
class Municipios extends Model
{
    ...
    public function buscar($request){
        $municipio = $this->select(['descripcion','id_municipio']);
        if ((!empty($request->q))){
            $usuario_empresa = UsuariosEmpresas::where('id_usuario', '=', Auth::user()->id)->first();
            $searchValue = $request->q;
            $departamento = $request->r;
            $municipio->where('id_empresa',$usuario_empresa->id_empresa);
            $municipio->where('id_departamento', $departamento);
            $municipio->where(function($query) use($searchValue){
                $query->where('descripcion', 'ILIKE','%'. $searchValue . '%');
            });
            $municipio->orderBy('id_municipio', 'asc');
            return $municipio->limit(6)->get();
        }else{
            $municipio->where('descripcion', 'ILIKE', '%Controls%');
            return $municipio->limit(0)->get();
        }
    }
