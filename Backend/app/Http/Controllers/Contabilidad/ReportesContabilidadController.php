...
//            $output = env('APP_URL_REPORTES') . $hora_actual .  'TasasCambio';
    ...
    /*
     * Reporte comparativo de salidas de inventario - contabilidad
     * @author octaviom
     * */
    public function generarReporteComparativoSalidasContabilidad($extension, $fecha_inicial, $fecha_final)
    {
        $os = array("pdf", "xls");
        //echo gethostname();
        if (in_array($extension, $os, true)) {
            ...
            $hora_actual = time();
            $nombre_reporte = 'ComparativoSalidasContabilidad';
...
/*            exec($jasper->process($input,$output,$options)->output().' 2>&1', $output);
    ...
    public function generarReporteFacturasDetallado($ext,$f_inicial,$f_final)
    {
        $os = array("xls", "pdf");
        if (in_array($ext, $os, true)) {
            // Obtener hora actual
            ...
            // Rutas de descarga de Reportes en servidor
            $input = env('APP_URL_REPORTES') . 'ReporteVentasDetallado';
            $output = env('APP_URL_REPORTES') . $hora_actual . 'ReporteVentasDetallado';
            ...
            return response()->download($output . '.' . $ext ,$hora_actual. 'FacturasDetallado.' . $ext, $headers)->deleteFileAfterSend();
...
//             Utilizar linea para depurar
     ...
     */
    public function obtenerCatalagoG (){
     ...
     * Obteniene datos que alimentan el formulario reporte de contabilidad
     */
    public function obtenerCatalagoRecibo (){
    ...
    public function generarReporteListadoRecibo($extension,$id_cliente, $fecha_inicial, $fecha_final)
    {
        $os = array("pdf", "xls");
        //echo gethostname();
        if (in_array($extension, $os, true)) {
            ...
            $hora_actual = time();
            $nombre_reporte = 'LitsadoDeRecibo';
