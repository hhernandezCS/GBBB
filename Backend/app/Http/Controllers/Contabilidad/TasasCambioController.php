...
class TasasCambioController extends Controller
{
    ...
    public function descargarTasasCambio(Request $request)
    {
        if (is_array($request->search['mes'])) {
            $mes = $request->search['mes']['mes'];
        } else {
            $mes = $request->search['mes'];
        }

        if (is_array($request->search['anio'])) {
            $anio = $request->search['anio']['periodo'];
        } else {
            $anio = $request->search['anio'];
        }
        if (!$mes || !$anio) {
            return response()->json([
                'status' => 'error',
        ...
        try {
            ...
            try {
                if (!empty($respuesta[0]) && $respuesta[0] !== 'Datos no encontrados') {
                    ...
                    return response()->json([
                        'status' => 'success',
                        'result' => ['Se han registrado ' . $cantidad . ' tasas de cambio para el mes ' . $mes . ' del año ' . $anio . '.'],
                        'messages' => null
                    ]);
                }
                ...
                return response()->json([
                    'status' => 'error',
                    'result' => ['No se han encontrado tasas de cambio para el mes ' . $mes . ' del año ' . $anio . '. Espere que sean publicadas en el sitio web del Banco Central'],
                    'messages' => null
                ]);
            ...
            return response()->json([
                'status' => 'error',
                'result' => ['Error al obtener las tasas de cambio'],
                'messages' => $e->getMessage()
            ]);
        }
    }
