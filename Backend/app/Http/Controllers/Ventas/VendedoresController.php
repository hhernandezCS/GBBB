...
/*            $vendedor = Vendedores::find($request->id_vendedor);*/
            $vendedor = Vendedores::select('public.users.name', 'venta.vendedores.*')
                ->join('public.users', 'public.users.id', '=', 'venta.vendedores.id_usuario')
                ->where('venta.vendedores.id_vendedor', $request->id_vendedor)
                ->first();
            if(!empty($vendedor)){
                return response()->json([
                   'status' => 'success',
    ...
    public function registrar(Request $request)
    {
        ...
        if (!$validator->fails()) {
            ...
            try{
                ...
                $vendedor->id_usuario = $request->user['id'];
                $vendedor->estado = 1;
                $vendedor->save();
    ...
    public function actualizar(Request $request)
    {
        ...
        if (!$validator->fails()) {
            ...
            $vendedor->email = $request->email;
            $vendedor->telefono = $request->telefono;
            $vendedor->id_usuario = $request->name['id'];
