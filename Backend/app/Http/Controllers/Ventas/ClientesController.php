...
class ClientesController extends Controller
{
    ...
    public function obtenerCliente(Request $request)
    {
        ...
        if (!$validator->fails()) {
            ...
            //$auxiliares = CatAuxiliar::select('*')->where('estado',1)->get();
...
/*            'numero_cedula.required_if' => 'El campo cédula es requerido para tipo de persona Natural',
        ...
        $rules = [
           ...
           // 'tipo_persona' => 'nullable|integer|min:1|max:2',
            //'nombre_completo' => 'nullable|string|max:100',
...
//            'numero_cedula' => 'nullable|required_if:tipo_persona,1|string|max:50|min:14',
         ...
         //   'direccion' => 'nullable|string|max:200',
            /*'tipo_cliente' => 'required|array|min:1',*/
         //   'id_tipo_cliente' => 'nullable|integer|min:1',
         //   'correo' => 'nullable|string|max:50',
           // 'departamento' => 'nullable|array|min:1',
          //  'departamento.id_departamento' => 'nullable|integer|min:1',
          //  'municipio' => 'nullable|array|min:1',
           // 'municipio.id_municipio' => 'nullable|integer|min:1',
          //  'vendedor' => 'nullable|array|min:1',
         //   'vendedor.id_vendedor' => 'nullable|integer|min:1',
         //   'giro_negocio' => 'nullable|string|max:100',
         //   'zona' => 'nullable|array|min:1',
         //   'zona.id_zona' => 'nullable|integer|min:1',
            /*'impuesto' => 'required|array|min:1',*/
        /*    'id_impuesto' => 'nullable|integer|min:1',
            'tipo_contribuyente' => 'nullable|integer|min:1|max:3',
            'retencion_ir' => 'nullable|boolean',
            'retencion_imi' => 'nullable|boolean',
            'clasificacion' => 'nullable|integer|min:1|max:2',
            'permite_credito' => 'nullable',
            'plazo_credito' => 'required_if:permite_credito,true|integer|min:0',
            'limite_credito' => 'required_if:permite_credito,true|numeric|regex:/^\d*(\.\d{1,2})?$/|min:0',
            'porcentaje_descuento' => 'nullable|numeric|regex:/^\d*(\.\d{1,2})?$/|min:0|max:50',
            'observaciones' => 'nullable|string|max:100',
            'permite_consignacion' => 'nullable|boolean',
            'tramite_cheque' => 'nullable|integer|min:0',*/
        ];
        $validator = Validator::make($request->all(), $rules, $messages);
        if (!$validator->fails()) {
            $cliente = new Clientes;
...
//            print_r('Codigo nuevo para cliente' . $str);
            ...
            if($request->municipio === null) {
                $cliente->id_municipio = 83;
            } else {
                $cliente->id_municipio = $request->municipio['id_municipio'];
            }

            if($request->vendedor === null) {
                $cliente->id_vendedor = 0;
            } else {
                $cliente->id_vendedor = $request->vendedor['id_vendedor'];
            }
            ...
            $cliente->tipo_contribuyente = $request->tipo_contribuyente;
            $cliente->retencion_ir = false;
            $cliente->retencion_imi = false;
            $cliente->clasificacion = $request->clasificacion;
...
//        $auxiliares = CatAuxiliar::select('*')->where('estado',1)->get();
    ...
    public function generarReporte($ext)
    {
        ...
        if (in_array($ext, $os)) {
            ...
            $input = ENV('APP_URL_REPORTES') . 'ListadoClientes';
            $output = ENV('APP_URL_REPORTES').$hora_actual.'ListadoClientes';
