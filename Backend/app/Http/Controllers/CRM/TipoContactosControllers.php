...
class TipoContactosControllers extends Controller
{
    /**
     * Obtener una lista de servicios
     * @access  public
     * @param Request $request
     * @param TipoContactos $tipo_contacto
     * @return JsonResponse
     * @author rsequeira
     */

    public function obtener(Request $request, TipoContactos $tipo_contacto)
    {
        $tipo_contacto = $tipo_contacto->obtener($request);
        return response()->json([
            'status' => 'success',
            'result' => [
                'total' => $tipo_contacto->total(),
                'rows' => $tipo_contacto->items()
            ],
            'messages' => null
        ]);
    }
    ...
    public function obtenerTipoContacto(Request $request)
    {
        $rules = [
            'id_tipo_contacto' => 'required|integer|min:1',
        ];

        $validator = Validator::make($request->all(), $rules);
        if (!$validator->fails()) {
            $tipo_contacto = TipoContactos::where('id_tipo_contacto',$request->id_tipo_contacto)->first();
            if(!empty($tipo_contacto)){
                return response()->json([
                    'status' => 'success',
                    'result' => $tipo_contacto,
                    'messages' => null
                ]);
            }
            else{
                return response()->json([
                    'status' => 'error',
                    'result' => array('id_tipo_contacto'=>["Datos no encontrados"]),
                    'messages' => null
                ]);
            }
        } else {
    ...
    public function registrar(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), $rules);
        if (!$validator->fails()) {
            $tipo_contacto = new TipoContactos();
            $tipo_contacto->descripcion = $request->descripcion;
            $tipo_contacto->id_empresa = $usuario_empresa->id_empresa;
            $tipo_contacto->estado = 1;
            $tipo_contacto->u_creacion = Auth::user()->name;
            $tipo_contacto->save();
    ...
    public function actualizar(Request $request)
    {
        $rules = [
            ...
            'descripcion' =>  [
                ...
                'string',
                'max:50',
                Rule::unique('pgsql.crm.tipo_contactos')->ignore($request->id_tipo_contacto,'id_tipo_contacto')],
        ];

        $validator = Validator::make($request->all(), $rules);
        if (!$validator->fails()) {
            $tipoContacto = TipoContactos::find($request->id_tipo_contacto);
            $tipoContacto->descripcion = $request->descripcion;
            $tipoContacto->u_modificacion = Auth::user()->name;
            $tipoContacto->save();
    ...
    public function desactivar(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), $rules);
        if (!$validator->fails()) {
            $tipo_contacto = TipoContactos::find($request->id_tipo_contacto);
