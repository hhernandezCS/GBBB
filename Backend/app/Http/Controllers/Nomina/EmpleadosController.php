...
use Hash, Validator;
use App\Models\Nomina\Personas;
...
class EmpleadosController extends Controller
{
    ...
    public function obtenerEmpleados(Request $request, Empleados $empleado)
    {
    ...
    public function buscarEmpleados(Request $request, Empleados $empleado)
    {
    ...
    public function obtenerEmpleado(Request $request, Empleados $empleado)
    {
        $rules = [
	...
	public function actualizarEmpleado(Request $request)
	{
		...
		if (!$validator->fails()) {
			...
			$usuario = UsuariosEmpresas::findOrFail($request->id_usuario);
			if($request->password!=''){
			$usuario->password = Hash::make($request->password);
			}
			$usuario->id_rol = $request->id_rol;
			...
			$empleado = Empleados::findOrFail($usuario->id_empleado);
			//$empleado->codigo = $request->codigo;
			//$empleado->save();

			$persona = Personas::findOrFail($empleado->id_persona);
			$persona->nombre = $request->nombre;
			$persona->primer_apellido = $request->primer_apellido;
			if(!empty( $request->segundo_apellido)){
				$persona->segundo_apellido = $request->segundo_apellido;
	...
	public function cambiarContrasenaEmpleado(Request $request)
	{
		...
		$validator = Validator::make($request->all(), $rules);
		if (!$validator->fails()) {
			$user = UsuariosEmpresas::find($request->id);
	...
	public function desactivarEmpleado(Request $request)
	{
		...
		if (!$validator->fails()) {
			...
			$usuario = UsuariosEmpresas::where('id_empleado','=', $empleado->id_empleado)->get()->first();;
			//print_r($usuario);
