...
use App\Models\Nomina\Feriados;
use App\Models\Nomina\IngresosDeducciones;
use App\Models\Nomina\IngresosDeduccionesTrabajadores;
use App\Models\Nomina\Marcadas;
use App\Models\Nomina\PlanillaControl;
use App\Models\Nomina\SaldosVacaciones;
use App\Models\Nomina\SolicitudVacaciones;
use App\Models\Nomina\SolicitudVacacionesDetalle;
use App\Models\Nomina\TiposJustificacionesMarcadas;
...
class TiposJustificacionesMarcadasController extends Controller
{
    /**
     ...
     * @param TiposJustificacionesMarcadas $solicitud
     * @return  \Illuminate\Http\JsonResponse(array)
     */

    public function obtener(Request $request, TiposJustificacionesMarcadas $solicitud)
    {
     ...
     * @access  public
     * @param Request $request
     * @param Marcadas $solicitud
     * @return  \Illuminate\Http\JsonResponse(array)
     */

    public function obtenerTodas(Request $request, TiposJustificacionesMarcadas $solicitud)
    {
        $solicitud = TiposJustificacionesMarcadas::orderby('id_tipo_marcada_justificacion')->get();
    ...
    public function obtenerJustificacion(Request $request)
    {
        ...
        if (!$validator->fails()) {
            $solicitud = TiposJustificacionesMarcadas::where('id_tipo_marcada_justificacion',$request->id_tipo_marcada_justificacion)->first();
    ...
    {
        $trabajadores = Trabajadores::select('id_trabajador','primer_apellido','primer_nombre','segundo_apellido','segundo_nombre','codigo',
        ...
        $planillas = PlanillaControl::select('id_planilla_control','id_sucursal','codigo_planilla','tipo_planilla','descripcion',
            DB::raw("CONCAT(descripcion,'(',codigo_planilla,')' )AS planilla"))->get();

        $ingresos = IngresosDeducciones::select('id_cat_ingreso_deduccion','cve_ingreso_deduccion','descripcion','codigo',
            DB::raw("CONCAT(descripcion,'(',cve_ingreso_deduccion,')' ) AS Ingreso"))->get();

        if(!empty($request->id_planilla_control))
        {
            $planilla = PlanillaControl::select('id_planilla_control','id_sucursal','codigo_planilla','tipo_planilla','descripcion',
    ...
    public function obtenerPlanilla(Request $request)
    {
        ...
        if (!$validator->fails()) {
            ...
            $ingresos = IngresosDeduccionesTrabajadores::where('id_trabajador',$request->id_trabajador)->where('id_planilla_control',$request->id_planilla_control)->where('estado',1)->with('asignacionIngreso')->get();
    ...
    public function registrar(Request $request)
    {
        ...
        if (!$validator->fails()) {
            ...
            try {
              ...
              /*  $solicitud = new IngresosDeduccionesTrabajadores;
               $solicitud->id_planilla_control = $request['planilla']['id_planilla_control'];
                ...
                IngresosDeduccionesTrabajadores::where('id_cat_ingreso_deduccion_trabajador',$request->id_cat_ingreso_deduccion_trabajador)->delete();
                ...
                foreach ($request->detalleSolicitud as $comprobante_detalle) {
                    ...
                    if($comprobante_detalle['id_cat_ingreso_deduccion_trabajador'] == 0 && $comprobante_detalle['estado'] == 1) {
                        $detalles = new IngresosDeduccionesTrabajadores;
                    ...
                    if($comprobante_detalle['id_cat_ingreso_deduccion_trabajador'] > 0 && $comprobante_detalle['estado'] == 0) {
                        $detalles = IngresosDeduccionesTrabajadores::find($comprobante_detalle['id_cat_ingreso_deduccion_trabajador']);
    ...
    public function actualizar(Request $request)
    {
        ...
        if (!$validator->fails()) {
            ...
            try{
                ...
                $solicitud = SolicitudVacaciones::findOrFail($request->id_vacacion_solicitud);
                if($solicitud->estado == 1 || $solicitud->estado ==2){
                    ...
                    SolicitudVacacionesDetalle::where('id_vacacion_solicitud', $solicitud->id_vacacion_solicitud)->delete();

                    $i = 1;
                    foreach ($request->solicitud_detalle as $comprobante_detalle) {
                        $detalles = new SolicitudVacacionesDetalle();
    ...
    public function cambiarEstadoSolicitud(Request $request)
    {
        ...
        if (!$validator->fails()) {
            ...
            $solicitud = SolicitudVacaciones::find($request->id_vacacion_solicitud);
            ...
            if($solicitud->estado==1 && $request-> estado == 0){
                ...
                $saldo = SaldosVacaciones::where('id_trabajador', $solicitud->id_trabajador)->first();
    ...
    public function activar(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), $rules);
        if (!$validator->fails()) {
            $rol = Feriados::find($request->id_feriado);
            $rol->estado = 1;
            $rol->save();
