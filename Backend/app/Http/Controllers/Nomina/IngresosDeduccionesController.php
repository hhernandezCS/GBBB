...
class IngresosDeduccionesController extends Controller
{
     ...
     * @param IngresosDeducciones $deduccion
     * @return  \Illuminate\Http\JsonResponse(array)
     */

    public function obtenerIngresos(Request $request, IngresosDeducciones $deduccion)
    {
    ...
    public function obtenerIngresosDeducciones(Request $request, IngresosDeducciones $deduccion)
    {
    ...
    public function obtenerDeducciones(Request $request, IngresosDeducciones $deduccion)
    {
    ...
    public function obtenerTodas(Request $request, IngresosDeducciones $deduccion)
    {
        $deduccion = IngresosDeducciones::orderby('id_cat_ingreso_deduccion')->get();
        return response()->json([
            'status' => 'success',
            'result' => $deduccion,
            'messages' => null
    ...
    public function obtenerIngresoDeduccion(Request $request)
    {
        ...
        if (!$validator->fails()) {
            $deduccion = IngresosDeducciones::where('id_cat_ingreso_deduccion',$request->id_cat_ingreso_deduccion)->with('ingresoDeduccionCuentaContable','ingresoDeduccionCuentaContableAdministrativa')->first();
            $cuenta_contable = CuentasContables::select('id_cuenta_contable','nombre_cuenta','codigo_cuenta')->get();
            if(!empty($deduccion)){
            return response()->json([
                'status' => 'success',
                'result' => [
    ...
    public function registrar(Request $request)
    {
        ...
        if (!$validator->fails()) {
            ...
            $secuencia = IngresosDeducciones::select(DB::raw('max(codigo) as codigo' )) -> where('cve_ingreso_deduccion',$request->cve_ingreso_deduccion)->first();
            $deduccion = new IngresosDeducciones();
    ...
    public function actualizar(Request $request)
    {
        ...
        if (!$validator->fails()) {
            ...
            $deduccion = IngresosDeducciones::find($request->id_cat_ingreso_deduccion);
            $deduccion->cve_ingreso_deduccion = $request->cve_ingreso_deduccion;
            $deduccion->descripcion = strtoupper($request->descripcion);
            $deduccion->orden = $request->orden;
            $deduccion->abreviacion = strtoupper($request->abreviacion);
    ...
    public function activar(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), $rules);
        if (!$validator->fails()) {
            $rol = IngresosDeducciones::find($request->id_cat_ingreso_deduccion);
            $rol->estado = 1;
            $rol->save();
