...
use App\Models\Nomina\DatosMedicos;
use App\Models\Nomina\GenerarPlanilla;
use App\Models\Nomina\PlanillaControl;
use App\Models\Nomina\PlanillaHistorico;
use App\Models\Nomina\PlanillasControles;
...
class GenerarPlanillasController extends Controller
{
     ...
     * @param GenerarPlanilla $planilla
     * @return JsonResponse
     */

    public function obtenerPlanilla(Request $request, GenerarPlanilla $planilla)
    {
        $rules = [
    ...
    public function obtenerPlanillaProcesar(Request $request)
    {
        ...
        if (!$validator->fails()) {
            $planilla = GenerarPlanilla::where('id_planilla_control', $request->id_planilla_control)->get();
            $control_planilla = PlanillasControles::where('id_planilla_control',$request->id_planilla_control)->first();

            $total_salarios_administrativo = GenerarPlanilla::select(DB::raw("SUM(monto) as total_salarios"))
            ...
            $total_salarios_comercial = GenerarPlanilla::select(DB::raw("SUM(monto) as total_salarios"))
                ->join('rrhh.trabajadores','rrhh.planillas.id_trabajador','rrhh.trabajadores.id_trabajador')->where('id_planilla_control',$request->id_planilla_control)
            ...
            $total_vacaciones_comercial = GenerarPlanilla::select(DB::raw("SUM(monto) as total_vacaciones"))
                ->join('rrhh.trabajadores','rrhh.planillas.id_trabajador','rrhh.trabajadores.id_trabajador')->where('id_planilla_control',$request->id_planilla_control)
            ...
            $total_treceavomes_administrativo = GenerarPlanilla::select(DB::raw("SUM(monto) as total_treceavomes"))
                ->join('rrhh.trabajadores','rrhh.planillas.id_trabajador','rrhh.trabajadores.id_trabajador')->where('id_planilla_control',$request->id_planilla_control)
            ...
            $total_treceavomes_comercial = GenerarPlanilla::select(DB::raw("SUM(monto) as total_treceavomes"))
                ->join('rrhh.trabajadores','rrhh.planillas.id_trabajador','rrhh.trabajadores.id_trabajador')->where('id_planilla_control',$request->id_planilla_control)
            ...
            $total_comisiones_administrativo = GenerarPlanilla::select(DB::raw("SUM(monto) as total_comisiones"))
                ->join('rrhh.trabajadores','rrhh.planillas.id_trabajador','rrhh.trabajadores.id_trabajador')->where('id_planilla_control',$request->id_planilla_control)
            ...
            $total_comisiones_comercial = GenerarPlanilla::select(DB::raw("SUM(monto) as total_comisiones"))
                ->join('rrhh.trabajadores','rrhh.planillas.id_trabajador','rrhh.trabajadores.id_trabajador')->where('id_planilla_control',$request->id_planilla_control)
            ...
            $total_general = GenerarPlanilla::select(DB::raw("SUM(monto) as monto_neto"))->where('id_planilla_control',$request->id_planilla_control)->get();
    ...
    public function registrar(Request $request) //Guardar datos de la planilla generada
    {
        ...
        if (!$validator->fails())
        {
            try{
                ...
                $control_de_planilla = PlanillaControl::select('id_planilla_control','estado','f_modificacion','u_modificacion')->where('id_planilla_control',$request->id_planilla_control)->first();
                ...
                GenerarPlanilla::where('id_planilla_control',$request->id_planilla_control)->delete();
                //Guardar datos de la planilla
                foreach ($request->planilla as $planillax) {

                    $planilla =  new GenerarPlanilla();
                    $planilla_control = PlanillaControl::select('id_planilla_control','anio_proceso','mes_proceso','f_desde','f_hasta')->where('id_planilla_control',$request->id_planilla_control)->first();
        ...
        if (!$validator->fails())
        {
            try {
                ...
                foreach ($request->planilla as $planillax) {
                    ...
                    $planilla_historico =  new PlanillaHistorico();
                    $planilla_control = PlanillaControl::select('id_planilla_control','anio_proceso','mes_proceso','f_desde','f_hasta')->where('id_planilla_control',$request->id_planilla_control)->first();
                ...
                $total_general = GenerarPlanilla::select(DB::raw("SUM(monto) as monto_neto"))->where('id_planilla_control',$planillax['id_planilla_control'])->first();
                ...
                $total_horas_extras_administrativo = GenerarPlanilla::select(DB::raw("SUM(monto) as total_horas"))
                    ->join('rrhh.trabajadores','rrhh.planillas.id_trabajador','rrhh.trabajadores.id_trabajador')->where('id_planilla_control',$planillax['id_planilla_control'])
                ...
                $total_inatec_administrativo = GenerarPlanilla::select(DB::raw("SUM(monto) as total_inactec"))
                    ->join('rrhh.trabajadores','rrhh.planillas.id_trabajador','rrhh.trabajadores.id_trabajador')->where('id_planilla_control',$planillax['id_planilla_control'])
                ...
                $total_horas_extras_comercial = GenerarPlanilla::select(DB::raw("SUM(monto) as total_horas"))
                    ->join('rrhh.trabajadores','rrhh.planillas.id_trabajador','rrhh.trabajadores.id_trabajador')->where('id_planilla_control',$planillax['id_planilla_control'])
                ...
                $total_inatec_comercial = GenerarPlanilla::select(DB::raw("SUM(monto) as total_inactec"))
                    ->join('rrhh.trabajadores','rrhh.planillas.id_trabajador','rrhh.trabajadores.id_trabajador')->where('id_planilla_control',$planillax['id_planilla_control'])
                ...
                $total_ir = GenerarPlanilla::select(DB::raw("SUM(monto) as total_ir"))
                    ->join('rrhh.trabajadores','rrhh.planillas.id_trabajador','rrhh.trabajadores.id_trabajador')->where('id_planilla_control',$planillax['id_planilla_control'])
                ...
                $total_inss_patronal = GenerarPlanilla::select(DB::raw("SUM(monto) as total_inss_patronal"))
                    ->join('rrhh.trabajadores','rrhh.planillas.id_trabajador','rrhh.trabajadores.id_trabajador')->where('id_planilla_control',$planillax['id_planilla_control'])
    ...
    {
        $planillas_controles = PlanillaControl::select(['id_planilla_control','codigo_planilla','tipo_planilla','descripcion','anio_proceso','mes_proceso','estado'])->whereIn('estado',array(1,2))->get();
