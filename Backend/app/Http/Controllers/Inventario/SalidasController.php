...
//                    $bodega_sub->save();
    ...
    public function nueva(Request $request)
    {
        ...
        $tipos_salida = TipoSalida::where('estado', 1)->select('id_tipo_salida', 'descripcion')->where('estado', 1)/*->whereIn('id_tipo_salida', array(2,5,7))*/ ->get();

        if (Auth::user()->id_sucursal > 0) {
            $bodegas = Bodegas::select('id_bodega', 'descripcion', 'estado', 'id_tipo_bodega')->where('estado', 1)->whereNotIn('id_tipo_bodega', array(6, 4))
                ->where('id_sucursal', Auth::user()->id_sucursal)
                ->orderby('descripcion')->get();
        } else {
            $bodegas = Bodegas::select('id_bodega', 'descripcion', 'estado', 'id_tipo_bodega')->where('estado', 1)->whereNotIn('id_tipo_bodega', array(6, 4))->orderby('descripcion')->get();
        }
        ...
        if (!$validator->fails()) {
            ...
            try {
                ...
                if ($salida->estado === 1 || $salida->estado === 99) {
                    ...
                    $salida->fecha_despacho = $salida->fecha_salida; //date("Y-m-d H:i:s");
                    $salida->u_despacho = Auth::user()->name;
                    $salida->estado = 2;
                    $salida->save();
...
//                        print_r($costo_promediox['costo_promedio']);
                        ...
                        $movimiento_producto->tipo_movimiento = 2;
                        $movimiento_producto->cantidad_movimiento = $producto['cantidad_despachada'] * -1;
                        $movimiento_producto->costo = $producto_costo_nuevo['costo_estandar'];
                        $movimiento_producto->costo_me = $producto_costo_nuevo['costo_estandar_me'];
                        $movimiento_producto->usuario_registra = Auth::user()->name;
...
//            $output = env('APP_URL_REPORTES') . $hora_actual .  'ReporteEntradainventario';
    ...
    public function registrarDevolucion(Request $request)
    {
        ...
        $rules = [
            ...
            // 'salida_productos.*.id_producto' => 'required|integer|exists:pgsql.inventario.productos,id_producto',
            'salida_productos.*.id_bodega_producto' => 'required|integer|min:1|exists:pgsql.inventario.bodega_productos,id_bodega_producto',
            'salida_productos.*.cantidad_despachada' => 'required|numeric|min:0.01',
            'salida_productos.*.cantidad_devuelta' => 'required|numeric|min:0.01|lte:salida_productos.*.cantidad_despachada',
        ];
        ...
        if (!$validator->fails()) {
            ...
            try {
                ...
                if ($salida->estado === 2) {
                    ...
                    // Anular factura
                    $factura = Facturas::Where('id_salida',$salida->id_salida)->first();
                    $factura->estado = 0;
                    $factura->save();

                    // Anular comprobantes de factura e inventario
                    $comprobante_factura = DocumentosContables::find($factura->id_documento_contable);
                    $comprobante_factura->estado = 0;
                    $comprobante_factura->save();

                    $comprobante_inventario = DocumentosContables::find($salida->id_documento_contable);
                    if($comprobante_inventario){
                        $comprobante_inventario->estado = 0;
                        $comprobante_inventario->save();
                    }
...
//                    print_r($log['registro'].' - '.$log['fecha_log']);
                    ...
                    foreach ($request->salida_productos as $producto) {
                        ...
                        if ($producto['cantidad_devuelta'] > 0) {
                            ...
                            $movimiento_producto->costo = $costo_promediox['costo_promedio'];
                            $movimiento_producto->costo_me = $costo_promediox['costo_promedio_me'];
                            $movimiento_producto->usuario_registra = Auth::user()->name;
                            $movimiento_producto->save();
                        }
                    }
