                    ...
                    if ($producto['estado'] === 0) {
                        ...
                        if (!$producto['registrada']) {
                            if (empty($producto['id_entrada_producto'])) {
                                ...
                                $bodega_sub = BodegasProductos::where('id_bodega', $request->bodega['id_bodega'])->where('id_producto', $producto['productox']['id_producto'])->where('no_documento', 'GENERAL')->first();
                                if (!empty($bodega_sub)) {
                                    ...
                                    $nueva_bodega_sub->no_documento = 'GENERAL';
                                ...
                                $producto_individual->precio_unitario = $producto['precio_unitario'];
                        ...
                        if ($producto['estado'] === 0) { //Productos eliminados
                            ...
                            if (!$producto['registrada']) {
                                ...
                                if (empty($producto['id_entrada_producto'])) {
                                    ...
                                    $bodega_sub = BodegasProductos::where('id_bodega', $request->entrada_bodega['id_bodega'])->where('id_producto', $producto['bodega_producto']['id_producto'])->where('no_documento', 'GENERAL')->first();
                                    ...
                                    $entrada_producto->precio_unitario_me = $producto['precio_unitario'] / $request->t_cambio;
                                    $entrada_producto->precio_unitario = $producto['precio_unitario'];
                                    ...
                                    $producto_individual->precio_unitario_me = $producto['precio_unitario'] / $request->t_cambio;
                                    $producto_individual->precio_unitario = $producto['precio_unitario'];
...
//                            EntradaProductos::where('id_entrada_producto', $producto_registrado->id_entrada_producto)->where('id_empresa', $usuario_empresa->id_empresa)->delete();
                        ...
                        $bodega_sub = BodegasProductos::where('id_bodega', $request->id_bodega)->where('id_producto', $request->id_producto)->where('id_empresa', $usuario_empresa->id_empresa)->where('no_documento', 'GENERAL')->first();
                        if (!empty($bodega_sub)) {
