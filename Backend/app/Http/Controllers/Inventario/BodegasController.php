...
class BodegasController extends Controller
{
    ...
    public function obtenerExistenciaMinima(Request $request)
    {


        $productosBodega = DB::table('inventario.productos as pd')
            ->select('pd.existencia_min as existencia_minima', 'bp.cantidad as stock')
            ->join('inventario.bodega_productos as bp', 'pd.id_producto', '=', 'bp.id_producto')
            ->get();
        ...
        return response()->json([
            ...
            'result' => [
                ...
                'existenciacantidad' => $productosBodega,
    ...
    public function obtenerBodegaProductosVenta(Request $request, BodegaProductos $productos, ProductosVistaVenta $producto_venta)
    {
        ...
        if (!empty($request->is_carne) && $request->is_carne === 'A') {
            ...
            $cantProducto = array_splice($arrEnt, 7, 11);
            array_pop($cantProducto);
            $result = implode('', $cantProducto);
            $result = substr($result, 0, -3) . "." . substr($result, -3);
            $result = floatval($result);

            $producto_venta = $producto_venta->productosBodegaVenta($request);
            if (empty($producto_venta[0])) {
                $producto_venta = [];

                return response()->json([
                    'results' => $producto_venta

                ]);
            } else {
                $producto_venta[0]->cantP = $result;
                return response()->json([
                    'results' => $producto_venta
