...
//        $bodegas =Bodegas::where('activo', 1)->select('id_bodega','descripcion')->get();
  //    $impuestos = PublicImpuestos::select(['id_impuesto','descripcion'])->get();
        $grupos = Grupos::select(['id_grupo','descripcion','codigo'])->orderby('codigo')->where('estado',1)->get();
...
//               'impuestos'=> $impuestos,
    ...
    {
        $productos = $productos->buscar($request);
        return response()->json([
            'results' => $productos
        ]);
    }
...
/*            'nombre_comercial.required' => 'El campo nombre del producto es requerido.',*/
            ...
            'grupo.required' => 'El campo grupo de producto es requerido.',
            ...
            'codigo_barra.unique' => 'Ya existe un producto registrado con este codigo de barras.'
        ];
        $rules = [
...
/*            'precio_distribuidor' => 'required|numeric|regex:/^\d*(\.\d{1,2})?$/',*/
            ...
            'codigo_barra'=> 'string|max:50|nullable|unique:pgsql.inventario.productos,codigo_barra',
...
//                DB::beginTransaction();
                ...
                if ($request->codigo_barra === null) {
                    $aleatorio = mt_rand(1, 9999); // Generar número aleatorio de 4 dígitos
                    $codigo_barra = str_pad(mt_rand(1, 9999), 4, '0', STR_PAD_LEFT); // Combinar el código del sistema con el número aleatorio y agregar ceros a la izquierda
                } else {
                    $codigo_barra = $request->codigo_barra;
                }

                $producto->codigo_barra = $codigo_barra;
                $producto->nombre_comercial = strtoupper($request->descripcion);
                $producto->descripcion = strtoupper($request->descripcion);
                $producto->precio_distribuidor = $request->precio_distribuidor;
                $producto->precio_compra = $request->precio_compra;
                $producto->costo_estandar = $request->precio_compra;
                $producto->costo_estandar_me = $request->precio_compra;
                $producto->precio_sugerido = $request->precio_sugerido;
                $producto->id_unidad_medida = $request->unidad_medida['id_unidad_medida'];
                $producto->id_grupo = $request->grupo['id_grupo'];
                $producto->id_subgrupo = optional($request->subgrupo)['id_subgrupo'];
                $producto->id_marca = optional($request->marca)['id_marca'];
            ...
            /*    if ($request->tipo_producto === 1){|
              ...
              /*  $producto->codigo_barra = $request->codigo_barra;*/
                ...
                if($request->tipo_producto === 1 && $producto->cantidad_inicial > 0) {
                    ...
                    $entrada->descripcion_entrada = 'Registramos Inventario Inicial del producto  ' . $producto->descripcion;
...
//                Rule::unique('pgsql.inventario.productos')->ignore($request->id_producto,'id_producto')
            ...
            'costo_estandar' => 'required|numeric|regex:/^\d*(\.\d{1,6})?$/',
            'precio_sugerido' => 'required|numeric|min:1|regex:/^\d*(\.\d{1,2})?$/',
            'precio_compra' => 'required|numeric|regex:/^\d*(\.\d{1,2})?$/',
            'precio_distribuidor' => 'required|numeric|min:1|regex:/^\d*(\.\d{1,2})?$/',
...
/*            'marca.id_marca' => 'required_if:id_tipo_producto,1|integer|min:0',*/
        ...
        if (!$validator->fails()) {
            ...
            try{
                ...
                $producto->codigo_sistema = $request->codigo_sistema;
                $producto->nombre_comercial = $request->descripcion;
