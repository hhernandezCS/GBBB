        ...
        if (!$validator->fails()) {
            ...
            if (!empty($factura)) {
                ...
                if ($request->cargar_dependencias) {
                    $ajustes_basicos = Ajustes::whereIn('id_ajuste', array(4, 5, 11,6))->orderBy('id_ajuste')->select('id_ajuste', 'valor')->get();
...
//                $documento_cuenta_contableS2a->debe_org = $factura->mt_deuda - $factura->mt_retencion - $factura->mt_retencion_imi; /*-$factura->mt_descuento*/
    ...
    public function registrar(Request $request)
    {
        ...
        $rules = [
            ...
            'aplicaIMI' => 'required|boolean',
//
//            '   ' => 'required_if:es_nuevo,==,true|array|min:1',
//            'factura_cliente.id_cliente' => 'required_if:es_nuevo,==,true|integer|min:1',
            ...
            'detalleProductos.*.productox.codigo_sistema' => 'required|string|max:50',
            'detalleProductos.*.productox.descripcion' => 'required|string|max:300',
            'detalleProductos.*.productox.unidad_medida' => 'required|string|max:100',
            'detalleProductos.*.afectacionx.id_afectacion' => 'required|integer|exists:pgsql.cjabnco.facturas_afectaciones,id_afectacion',
            'detalleProductos.*.afectacionx.valor' => 'required|numeric|regex:/^\d*(\.\d{1,2})?$/',
...
//                    $factura->no_factura = $sucursalx2['numero_facturacion'] + 1;
                ...
                $sucursalx->numero_facturacion = $factura->no_factura;
                $sucursalx->save();
                ...
                $salida->id_cliente = $factura->id_cliente;
                $salida->fecha_salida = $factura->f_factura;
                $salida->fecha_despacho = $factura->f_factura;
                $salida->numero_documento = $factura->no_documento;
                $salida->id_bodega = $factura->id_bodega;
                $salida->descripcion_salida = $request->factura_cliente['nombre_comercial'] . ' Fact. No.' . $factura->no_documento;
                $salida->u_creacion = $factura->u_creacion;
                $salida->u_despacho = $factura->u_creacion;
                $salida->estado =2; // Despachado
...
//                Sucursales::find($factura->id_sucursal)->increment('numero_facturacion');
                ...
                #endregion seccion proforma

                #region seccion cuentas por cobrar
                ...
                /**
                 * Reducimos saldo o cancelamos cuenta por cobrar de cliente
                 *
                 */
                //Facturas tipo ANTICIPO
                if ($request->id_tipo === 4 || $request->id_tipo === '4') {
                    foreach ($request->detalleCuentasxCobrar as $recibosDetalles) {
                        $cuenta_x_cobrar_update = CuentasXCobrar::findOrFail($recibosDetalles['cuentax']['id_cuentaxcobrar']);
                        $recibo_update = Recibos::findOrFail($recibosDetalles['cuentax']['identificador_origen']);

                        $saldo_actual_cxc = $cuenta_x_cobrar_update->saldo_actual - $recibosDetalles['monto'];
                        $saldo_actual_cxc_me = $cuenta_x_cobrar_update->saldo_actual_me - $recibosDetalles['monto_me'];
                        //Validar si
                        if (abs($saldo_actual_cxc) === 0 || abs($saldo_actual_cxc_me) === 0) {
                            $cuenta_x_cobrar_update->saldo_actual = 0;
                            $cuenta_x_cobrar_update->saldo_actual_me = 0;
                            $cuenta_x_cobrar_update->estado = 2; // Cancelada
                            $cuenta_x_cobrar_update->save();
                            $recibo_update->estado = 3; //Cancelado por factura anticipo
                            $recibo_update->save();
                        }else {
                            $cuenta_x_cobrar_update->saldo_actual = $saldo_actual_cxc;
                            $cuenta_x_cobrar_update->saldo_actual_me = $saldo_actual_cxc_me;
                            $cuenta_x_cobrar_update->save();
                        }
                        $recibo_update->estado = 3; //Cancelado por factura anticipo
                        $recibo_update->save();
                ...
                #region seccion detalle de pago
                foreach ($request->detallePago as $pago) {
                ...
                #endregion seccion detalle de pago
                ...
                foreach ($request->detalleProductos as $producto) {
                    ...
                    $factura_producto->precio_costo_me = $producto['precio_costo_me'];;
                    ...
                    if ($producto['productox']['id_tipo_producto'] !== 2) {
                        ...
                        if (!$productoExiste) {
                            ...
                            $salida_producto->cantidad_saliente = $producto['cantidad'];
                            $salida_producto->cantidad_despachada = $producto['cantidad'];
                            $salida_producto->cantidad_faltante = 0;
                            $salida_producto->u_creacion = $salida->u_creacion;
                            $salida_producto->id_empresa = $usuario_empresa->id_empresa;
                            $salida_producto->save();
                        } else {
                            $productoExiste->cantidad_saliente += $producto['cantidad'];
                            $productoExiste->cantidad_despachada += $producto['cantidad'];
                        ...
                        $bodega_sub2 = BodegaProductos::where('id_bodega_producto', $salida_producto->id_bodega_producto)->first();
//                        print_r($bodega_sub2);
                        // Realizando calculos para despacho de productos
                        $bodega_sub2->cantidad -= $salida_producto->cantidad_despachada;
//                        print_r($bodega_sub2->cantidad -= $producto['cantidad_despachada']);
                        ...
                        $movimiento_producto->descripcion_movimiento = 'Salida por venta' . ' No. ' . $salida->numero_documento;
                        $movimiento_producto->identificador_origen_movimiento = $salida->id_salida;
                        $movimiento_producto->tipo_movimiento = 2;
                        $movimiento_producto->cantidad_movimiento = $salida_producto->cantidad_despachada * -1;
                        $movimiento_producto->costo =  $producto['precio_costo'];
                        $movimiento_producto->costo_me =  $producto['precio_costo_me'];
                ...
                #endregion detalle de productos
...
//                    }
                ...
                #endregion
                #endregion [Contabilización factura]
    ...
    public function obtenerUltimaFactura($idUltimaFactura = null): JsonResponse
    {
        if (!is_null($idUltimaFactura)) {
            return response()->json([
                'status' => 'success',
                'result' => [
                    'factura_id' => $idUltimaFactura,
                ],
                'messages' => null
            ]);
        } else {
    ...
    public function nueva(Request $request)
    {
        ...
        $vendedores = Vendedores::select('public.users.name', 'venta.vendedores.*')
            ->join('public.users', 'public.users.id', '=', 'venta.vendedores.id_usuario')
            ->where('venta.vendedores.estado', 1)
            ->where('venta.vendedores.id_usuario', Auth::user()->id)
            ->get();
...
/*        $vendedor_actual = Vendedores::select(['id_vendedor', 'nombre_completo', 'id_zona'])->where('estado', 1)->first();*/
   ...
   /*             'vendedor_actual' => $vendedor_actual,*/
    ...
    public function nuevaFacturaAjuste(Request $request)
    {
        ...
        $trabajador_actual = Trabajadores::select(['id_area'])->where('id_trabajador',$usuario_actual['id_empleado'])->where('activo',true)->first();
...
//            print_r( env('APP_URL_REPORTS').$logo_empresa->valor);
    ...
    public function anular(Request $request)
    {
        ...
        if (!$validator->fails()) {
            ...
            try {
                ...
                $factura->estado = 0;
                $factura->observacion = $factura['observacion'] . ' **Factura cancelada por ' . Auth::user()->name . ' a las ' . date("Y-m-d H:i:s") . ' Causa: ' . $request->causa_anulacion;
                $factura->save();
                    ...
                    if ($salida) {
                        ...
                        foreach ($factura->facturaProductos as $producto) {
                            ...
                            if (!empty($bodega_sub)) {
                                ...
                                } elseif ($factura->tipo_venta === 2) {
                                    $bodega_sub->cantidad += $producto['cantidad'];//salida venta recuperado
                                } elseif ($factura->tipo_venta === 4) {
                                    $bodega_sub->cantidad += $producto['cantidad'];//salida venta obsoleto
                                }
    ...
    public function enviarFactura (Request $request) {
        ...
        if($request->factura_cliente['tipo_persona'] === 1) {
            $identificacion = $request->factura_cliente['numero_cedula'];
        } else {
            $identificacion = $request->factura_cliente['numero_ruc'];
        }

        $condicion = 'Contado';

        $total_items = count($request->factura_productos);

        $monto = $request->factura_via_pago[0]['id_via_pago'] === 1 ? 'C$'.$request->factura_via_pago[0]['monto']:'$'.$request->factura_via_pago[0]['monto_me'];

        $token = 'EAAC53OZCkKQIBAPeMCpeYPZBZCyYp5gDTC7GWtws3rvvnRbOym7ONLZBKWZCIBtp470nZBUCsuM2oG2EPV4ZAfh7vb1c0rSMr1YWePeXkqeB89NblFB7H29EahQSEYSHaE7AKZALfYSjK67Jiy94C4ZCati7RDUEoFLS70OC0BZBqM0hRg8CAZAIjZAtdWFFOiN4wVZAu6f3h8kYh2gZDZD';

        $telefono = '50576501246';

        $url = 'https://graph.facebook.com/v16.0/100680419714383/messages';

        #region Estructura del mensaje con los datos de la factura
        $mensaje1 = ''
            .'{'
            .'"messaging_product": "whatsapp", '
            .'"to": "'.'505'.$request->factura_cliente['telefono'].'", '
            .'"type": "template", '
            .'"template": '
            .'{'
            .'"name": "facturacion_sanmartin", '
            .'"language":{ "code": "es" } '
            .'}, '
            .'"components": ['
            .'{'
            ...
            .'{'
            .'"type": "text",'
            .'"text": "'.$request->f_creacion.'", '
            .'}, '
            .'{'
            .'"type": "text",'
            .'"text": "'.$request->no_documento.'", '
            .'}, '
            .'{'
            .'"type": "text",'
            .'"text": "'.$moneda.'", '
            ...
            .'{'
            .'"type": "text",'
            .'"text": "'.$request->factura_cliente['nombre_comercial'].'", '
            .'}, '
            .'{'
            .'"type": "text",'
            .'"text": "'.$identificacion.'", '
            ...
            .'{'
            .'"type": "text",'
            .'"text": "'.$request->factura_vendedor['nombre_completo'].'", '
            .'}, '
            .'{'
            .'"type": "text",'
            .'"text": "'.$condicion.'", '
            ...
            .'{'
            .'"type": "text",'
            .'"text": "'.$request->t_cambio.'", '
            .'}, '
            .'{'
            .'"type": "text",'
            .'"text": "'.$request->mt_total.'", '
            .'}, '
            .'{'
            .'"type": "text",'
            .'"text": "'.$request->mt_total_me.'", '
            .'}, '
            .'{'
            .'"type": "text",'
            .'"text": "'.$request->factura_via_pago[0]['descripcion'].'", '
            .'}, '
            .'{'
            .'"type": "text",'
            .'"text": "'.$monto.'", '
        ...
        #region Plantilla de prueba por defecto
        $mensaje = ''
            .'{'
            .'"messaging_product": "whatsapp", '
            .'"to": "'.'505'.$request->factura_cliente['telefono'].'", '
