			...
			methods: {
				...
				confirmar(estadox){
					...
					if(estadox===0){
						...
						titulox = 'Esta seguro de cambiar el estado a Recibido?';
					}
				...
				obtenerOrdenCompra() {
					...
					orden.obtenerOrdenCompra({
						...
						if(self.form.estado === 99 || self.form.estado === 5 || self.form.estado === 0){
					...
					if( (self.form.estado === 1 && (estadox === 2|| estadox === 0)) ||
							(self.form.estado === 2 && (estadox === 3|| estadox === 0)) ||
								(self.form.estado === 3 && (estadox === 4|| estadox === 0)) ||
									(self.form.estado === 4 && (estadox === 5 || estadox === 0))){
						...
						orden.cambiarEstado({
							...
							data => {
								if(estadox === 2){
									...
									alertify.success("El estado de la orden de compra ha cambiado a Recibido",5);
								}
								this.$router.push({
									name: "listado-ordenes-compras-ps",
