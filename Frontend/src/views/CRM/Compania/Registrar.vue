...
<script type="text/ecmascript-6">
    ...
    export default {
        ...
        methods: {
            ...
            mapContactos() {
                return this.form.mapContactos = this.contactosMap.map((obj) => [obj.id_contacto]).join(",");
            },
            nuevo() {
                const self = this;
                self.loading = false;
            ...
            registrar() {
                this.$refs.companiaValidations.validate().then(succes => {
                    if (succes) {
                        ...
                        self.loading = true;
                        compania.registrar(self.form, data => {
                                self.loading = false;
                                this.$toast({
