        ...
        <validation-observer ref="ContactoValidations">
            <form>
                <div class="row">
                    ...
                    <template v-if="this.form.pais_contacto.id_pais === 21"> <!--Mostrar solo para USA-->
                        <div class="col-md-2">
                            <div class="form-group">
                                <validation-provider
                                        #default="{ errors }"
                    ...
                    <template v-if="this.form.pais_contacto.id_pais === 42"> <!--Mostrar solo para Nicaragua-->
                        <div class="col-md-2">
                            <div class="form-group">
                                <validation-provider
                                        #default="{ errors }"
                                ...
                                >
                                    ...
                                    <v-select :class="errors.length > 0 ? 'is-invalid':null"
                                              v-model="form.departamento_contacto"
                                              label="descripcion"
                        ...
                        <div class="col-md-2">
                            <div class="form-group">
                                ...
                                >
                                    ...
                                    <v-select :class="errors.length > 0 ? 'is-invalid':null"
                                              v-model="form.municipio_contacto"
                                              label="descripcion"
                                              :options="municipios"
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            <validation-provider
                                    ...
                                    rules="required"
                            >
                                <label for="">Clasificicacion del contaco</label>
                                <v-select
                                        :class="errors.length > 0 ? 'is-invalid':null"
                                        label="descripcion"
                    ...
                    <div>
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    <template v-if="this.form.pais_contacto.id_pais === 42">
                                        <b-form-input
                                                id="name-input2"
                                                v-model="tel_number2"
                                                :state="numberState2"
                    ...
                    <div class="col-sm-8 text-lg-right">
                        <template v-if="form.estado">
                            <b-button class="mt-1 mr-1" type="submit" variant="danger"
                                      @click="desactivar(form.id_contacto)">
                        ...
                        <template v-else>
                            <b-button class="mt-1 mr-1" type="submit" variant="success"
                                      @click="activar(form.id_contacto)">
        ...
        methods: {
            obtenerContactos() {
                ...
                contacto.obtenerContacto({
                        id_contacto: this.$route.params.id_contacto
                    },
                    data => {
                        self.form = data.contacto;
            ...
            obtenerCompanias() {
                let self = this;
                self.loading = true;
                contacto.obtenerCompanias({
                        id_contacto: this.$route.params.id_contacto
                    }, data => {
                        self.loading = false;
                        self.compania = data.companias;
                        if (this.compania) {
                            return this.form.mapCompanies = this.compania.map((compania) => [compania.id_compania]).join(",");
                        }
                    },
                    err => {
                        self.loading = false;
            ...
            activar(ContactoID) {
                ...
                self.$swal({
                    title: '¿Esta seguro?',
                    text: '¿ Desea activar el contacto?',
                    icon: 'info',
                    ...
                    if (result.value) {
                        ...
                        self.$swal({
                            title: 'Cancelado',
                            text: 'El contacto no ha sido habilitado',
                            icon: 'error',
                            customClass: {
                                confirmButton: 'btn btn-success',
                            },
            ...
            desactivar(id_contacto) {
                const self = this;
                self.$swal({
                    title: 'Info!',
                    text: '¿Esta seguro de desactivar el contacto?',
                    icon: 'info',
                    showCancelButton: true,
                    confirmButtonText: 'Si, confirmar',
                    cancelButtonText: 'Cancelar',
            ...
            mapCompanies() {
                if (this.compania) {
                    return this.form.mapCompanies = this.compania.map((obj) => [obj.id_compania]).join(",");
                }
                return this.form.mapCompanies = '';
            },
            ...
            searchD: _.debounce((loading, searchD, vm) => {
                const self = this;
                vm.form.municipio_contacto = {id_municpio: 0, descripcion: ''};
            ...
            searchE: _.debounce((loading, searchE, vm) => {
                const self = this;
                vm.form.ciudad_contacto = {id_ciudad: 0, descripcion: ''};
            ...
            searchM: _.debounce((loading, searchM, vm) => {
                const self = this;
                axios.get(`admon/municipios/buscar?q=${escape(searchM)}&r=${vm.form.departamento_contacto.id_departamento}`).then(res => {
                    vm.options = res.data.result;
            ...
            searchC: _.debounce((loading, searchC, vm) => {
                const self = this;
                axios.get(`admon/ciudades/buscar?q=${escape(searchC)}&r=${vm.form.estado_contacto.id_estado}`).then(res => {
                    vm.options = res.data.result;
            ...
            actualizar() {
                this.$refs.ContactoValidations.validate().then(succes => {
                    if (succes) {
                        ...
                        self.loading = true;
                        contacto.actualizar(self.form, data => {
                                self.loading = false;
                                this.$toast({
                                        component: ToastificationContent,
                                        props: {
