  ...
  <b-card>
    <b-row>
      <div class="col-md-3 sm-text-center">
        <router-link class="btn btn-success" tag="button"
                     :to="{ name: 'crm-medios-contacto-registrar' }">
      ...
      <div @keyup.enter="filter.page = 1;obtener();"
        ...
        <a hidden :disabled="descargando" @click.prevent="downloadItem('pdf')" style="color: #ffffff;padding-left: 2px"><b-button :disabled="descargando" variant="danger" v-b-tooltip.hover.top="'PDF'"><feather-icon icon="DownloadCloudIcon"></feather-icon></b-button></a>
        <a hidden :disabled="descargando" @click.prevent="downloadItem('xls')" style="color: #ffffff;padding-left: 2px"><b-button :disabled="descargando" variant="success" v-b-tooltip.hover.top="'XLS'"><feather-icon icon="DownloadCloudIcon"></feather-icon></b-button></a>
      </div>
    ...
    <div class="table-responsive mt-3">
      <table class="table table-striped table-bordered">
        ...
        <tbody>
        <template v-for="medio_contacto in mediosContacto">
          <tr :key="medio_contacto.id_medio_contacto">
            <td>{{ medio_contacto.descripcion }}</td>
            <td class="text-center">
              <div v-if="medio_contacto.estado===1">
            ...
            <td class="text-center">
              <template>
                <router-link v-b-tooltip="'Actualizar Medio de contacto'"
                             :to="{name: 'crm-medios-contacto-actualizar', params: {id_medio_contacto: medio_contacto.id_medio_contacto}}"
                             tag="a">
    ...
    <b-card-footer>
      <pagination @changePage="changePage" @changeLimit="changeLimit" :items="mediosContacto" :total="total"
...
import medioContacto from '../../../api/CRM/MediosContacto'
...
export default {
  ...
  methods: {
    ...
    obtener() {
      var self = this;
      self.loading = true;
      medioContacto.obtener(
          self.filter,
          data => {
            self.mediosContacto = data.rows;
