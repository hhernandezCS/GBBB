    ...
    <b-card>
        <b-row>
            <div class="col-md-4 sm-text-center">
                <router-link :to="{ name: 'admon-estados-registrar' }" class="btn btn-success" tag="button">
                    <i class="pe-7s-plus"></i> Registrar
                </router-link>
            </div>
            <div @keyup.enter="filter.page = 1;obtenerEstado();"
                ...
                <b-button class="btn btn-info mx-1" style="margin-right: .5rem!important;"
                          ...
                          @click="filter.page = 1; obtenerEstado();">
        ...
        <div class="table-responsive mt-3">
            <table class="table table-striped table-bordered">
                ...
                <tbody>
                <tr :key="estado.id_estado" v-for="estado in estados">
                    <td>{{ estado.descripcion }}</td>
                    <td class="text-center">{{ estado.estados_pais.codigo }}</td>
                    ...
                    <td class="text-center">
                        <router-link tag="a" :to="{ name: 'admon-estados-actualizar', params: { id_estado: estado.id_estado } }" v-b-tooltip.hover.top="'Editar registro!'"><feather-icon icon="EditIcon"></feather-icon></router-link>
                    </td>
                </tr>
        ...
        <b-card-footer>
            <pagination :items="estados" :limit="filter.limit" :limitOptions="filter.limitOptions" :page="filter.page"
    ...
    export default {
        ...
        methods: {
            obtenerEstado() {
                var self = this;
                self.loading = true;
                estado.obtener(self.filter, data => {
                    self.estados = data.rows;
