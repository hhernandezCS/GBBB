                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                >
                                    <b-form-input :state="errors.length > 0 ? false:null" class="form-control"
                                                  min="0"
                                                  type="number" v-model.number="item.precio_unitario"></b-form-input>
                                    <small class="text-danger">{{ errors[0] }}</small>
                                </validation-provider>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    computed: {
      ...
      total_subt() {
        return this.form.entrada_productos.reduce((carry, item) => {
          return (carry + Number(item.cantidad_solicitada) * Number(item.precio_unitario))
        }, 0)
      },
      gran_total() {
        return this.form.entrada_productos.reduce((carry, item) => {
          ...
          if (this.detalleForm.cantidad_solicitada > 0 && this.detalleForm.precio_unitario > 0) {
            ...
            if (i === 0) {
              this.form.entrada_productos.unshift({
                ...
                precio_unitario_me: Number(this.detalleForm.precio_unitario / this.form.t_cambio).toFixed(4),
                subtotal: 0,
                total: 0,
                registrada: false,
